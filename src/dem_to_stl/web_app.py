...
def _parameter_help() -> dict[str, str]:
    return {
        'center_lat': (
            'Center latitude in degrees. Lower values move south; higher '
            'values move north.'
        ),
        'center_lon': (
            'Center longitude in degrees. Lower values move west; higher '
            'values move east.'
        ),
        'radius_m': (
            'Selection radius in meters. Lower values select a smaller area '
            '(usually faster). Higher values select a larger area '
            '(usually slower).'
        ),
        'output_shape': (
            'XY footprint shape for the model: circular, hexagonal, or '
            'square.'
        ),
        'output_width_mm': (
            'Final model width in millimeters. Higher values enlarge X size '
            'and make slopes appear less steep.'
        ),
        'output_height_mm': (
            'Final model height in millimeters. Higher values enlarge Y '
            'size and make slopes appear less steep.'
        ),
        'vertical_exaggeration': (
            'Multiplicative Z scaling factor. Lower values flatten relief; '
            'higher values emphasize relief.'
        ),
        'mesh_spacing_mm': (
            'Baseline sample spacing in millimeters. Lower values increase '
            'detail and triangle count; higher values reduce detail and '
            'triangle count.'
        ),
        'base_height_mm': (
            'Constant base thickness in millimeters. Higher values create a '
            'thicker physical base.'
        ),
        'adaptive_triangulation': (
            'Enable adaptive refinement passes that add points where relief '
            'or triangle-angle quality requires it.'
        ),
        'adaptive_relief_threshold_mm': (
            'Per-triangle relief trigger in millimeters. Lower values '
            'refine more broadly; higher values refine only stronger terrain '
            'transitions.'
        ),
        'adaptive_iterations': (
            'Number of adaptive refinement passes. Lower values stop '
            'earlier; higher values continue refinement longer.'
        ),
        'adaptive_max_new_points': (
            'Global cap on adaptive point insertions. Lower values limit '
            'density/runtime; higher values allow denser local refinement.'
        ),
        'adaptive_min_angle_deg': (
            'Minimum triangle-angle target in degrees. Lower values allow '
            'skinnier triangles; higher values improve triangle shape '
            '(often adding more points).'
        ),
        'adaptive_anisotropic_refinement': (
            'Enable ridge-direction-aware refinement so inserted points can '
            'align along terrain structure.'
        ),
        'adaptive_anisotropic_strength': (
            'Strength of directional refinement. Lower values are subtle; '
            'higher values align refinement more strongly with ridge '
            'direction.'
        ),
        'earth_engine_project': (
            'Google Earth Engine project id used for authentication and '
            'quota context.'
        ),
        'dem_dataset_id': (
            'Earth Engine DEM asset id (Image or ImageCollection). '
            'ImageCollections are mosaiced before download.'
        ),
    }
...
def _serialize_job(record: JobRecord) -> dict[str, Any]:
    ...
    response: dict[str, Any] = {
        ...
        'dem_dataset_id': record.dem_dataset_id,
        'error': record.error,
...
def create_app() -> FastAPI:
    ...
    def init_db() -> None:
        with sqlite3.connect(db_path) as conn:
            ...
            if 'dem_dataset_id' not in columns:
                conn.execute('ALTER TABLE jobs ADD COLUMN dem_dataset_id TEXT')

            # Backfill explicit DEM id from stored request payload when available.
            rows = conn.execute(
                'SELECT job_id, request_json, dem_dataset_id FROM jobs',
            ).fetchall()
            for job_id, request_json, dem_dataset_id in rows:
                if dem_dataset_id:
                    continue
                try:
                    ...
                    req = {}
                dem_id = req.get('dem_dataset_id')
                if isinstance(dem_id, str) and dem_id.strip():
                    conn.execute(
                        'UPDATE jobs SET dem_dataset_id = ? WHERE job_id = ?',
                        (dem_id, job_id),
                    )
        ...
        with sqlite3.connect(db_path) as conn:
            conn.execute(
                ...
                INSERT INTO jobs (
                    ...
                    dem_dataset_id, output_path, geotiff_path, triangles, cache_hit,
                ...
                (
                    ...
                    record.dem_dataset_id,
                    str(record.output_path) if record.output_path is not None else None,
                    record.geotiff_path,
                    record.triangles,
                    1 if record.cache_hit else (
                        0 if record.cache_hit is not None else None
                    ),
    ...
    def load_jobs_from_db() -> None:
        ...
        for row in rows:
            request_dict = json.loads(
                row['request_json'],
            ) if row['request_json'] else {}
            dem_dataset_id = row['dem_dataset_id']
            if not dem_dataset_id:
                maybe_dem = request_dict.get('dem_dataset_id')
                if isinstance(maybe_dem, str) and maybe_dem.strip():
                    dem_dataset_id = maybe_dem
            ...
            jobs[row['job_id']] = JobRecord(
                ...
                cache_hit=(
                    None if row['cache_hit']
                    is None else bool(row['cache_hit'])
                ),
    ...
    async def run_job(job_id: str, payload: GenerateJobRequest) -> None:
        ...
        try:
            bbox = _bbox_from_center_radius(
                payload.center_lat, payload.center_lon, payload.radius_m,
            )
    ...
    async def list_jobs() -> list[dict[str, Any]]:
        ordered = sorted(
            jobs.values(), key=lambda rec: rec.created_at, reverse=True,
        )
    ...
    async def clear_cache() -> dict[str, int | str]:
        ...
        if cache_dir.exists():
            ...
            for folder in sorted(
                [p for p in cache_dir.rglob('*') if p.is_dir()],
    ...
    async def delete_job_stl(job_id: str) -> dict[str, str | bool]:
        ...
        if record.output_path is None:
            return {
                'job_id': job_id,
                'deleted': False,
                'reason': 'No output file tracked for this job.',
            }
        ...
        return {
            'job_id': job_id,
            'deleted': existed,
            'reason': 'Deleted' if existed else 'File already missing',
        }
